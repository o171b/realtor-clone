...
export default function ListingItem({listing,id}) {
  ...
  if (diff.days < -1 || diff.days > 1) {
    formattedDate += `${Math.abs(Math.round(diff.days))} days, `;
  }

  formattedDate += `${Math.abs(Math.round(diff.hours))} hours ago`;
  ...
  return (
    ...
    m-[10px]'>
      <Link className='contents' to={`/category/${listing.type}/${id}`}> 
        ...
        <div className='absolute top-2 left-2 bg-[#3377cc] text-white uppercase
        text-xs font-semibold rounded-md px-2 py-1 shadow-lg'>
          ...
          <div className="w-full p-[10px]">
            ...
            <p className='font-semibold m-0 text-xl truncate'>{listing.description}</p>
